...
function HistContainer(){
    ...
    useEffect(()=>{
        ...
        if (dataSetData === undefined || dataSetData === undefined || dataSetData.length == 0) {
            return;
        }

        const ara = dataSetData.map( x => { return x[dataSetData.selected]; }).sort();
        const counts = {};
        ...
        var output = Object.entries(result).map(([key, value]) => ({[dataSetData.selected] : key, Count : value}));
        console.log(output);

        var allAtributes = []
        if(!(dataSetData.data === undefined || dataSetData.data.length == 0))
        {
            allAtributes = Object.keys(dataSetData.data[0]);
            allAtributes.splice(0, 1);
        }
        console.log(allAtributes);

        histD3.renderHist(output,dataSetData.selected,yAttribute,allAtributes,controllerMethods);
    },[dataSetData]);// if dependencies, useEffect is called after each data update, in our case only dataSetData changes.
